<?php
  ...
  if(isset($_SESSION['id_admin'])){
    ...
    for($i=1; $i<=$row_hari; $i++){
      ...
      $result_produk[$i] = mysqli_query($con, "SELECT COUNT(*) as TOT_PRODUK FROM pesanan WHERE (STATUS_PESANAN = 4 OR STATUS_PESANAN = 5) AND TANGGAL_PESANAN LIKE '%$day%'");
      $data_produk[$i] = mysqli_fetch_assoc($result_produk[$i]);
      // var_dump($data_produk[$i]['TOT_PRODUK']);

      $result_omset[$i] = mysqli_query($con, "SELECT SUM(TOTAL_HARGA) as TOT_OMSET FROM pesanan WHERE (STATUS_PESANAN = 4 OR STATUS_PESANAN = 5) AND TANGGAL_PESANAN LIKE '%$day%'");
...
$pdf->Cell(190,10,'LAPORAN DATA BULAN '.strtoupper(month(date($bulan))).' '.$tahun,0,1,'C');
